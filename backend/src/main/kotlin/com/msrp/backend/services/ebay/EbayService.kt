...
class EbayService(
    ...
    companion object {
        ...
        private val SERP_CARD_SELECTORS =
            listOf(
    ...
    fun verifyGuess(
        ...
        val item =
            dailyItemRepository.findById(itemId).orElseThrow {
    ...
    private fun scrapeCompletedAuctions(keyword: String): List<DailyItem> {
        ...
        for (el in rows) {
            try {
                ...
                val cardBidCount =
                    BID_COUNT_REGEX.find(el.text())?.groupValues?.get(1)?.toIntOrNull()
                        ?: continue
    ...
    private fun ebayImagePathPixelHint(url: String): Int = Regex("""/s-l(\d+)""", RegexOption.IGNORE_CASE).find(url)?.groupValues?.get(1)?.toIntOrNull() ?: 0
